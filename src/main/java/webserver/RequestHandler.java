...
public class RequestHandler extends Thread {
	...
	public void run() {
		...
		try (OutputStream out = connection.getOutputStream(); BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()))) {
	...
	private void showUserList(Request request, Response response) {
	...
	private void loginProcess(Request request, Response response) throws IOException {
	...
	private void responseHeader(DataOutputStream dos, Response response) {
		try {
			...
			setContentLength(dos, response);
			setHeaders(dos, response);
			setCookies(dos, response);
			dos.writeBytes("\r\n");
		} catch (IOException e) {
			log.error(e.getMessage());
		}
	...
	private void setCookies(DataOutputStream dos, Response response) throws IOException {
	...
	private void setHeaders(DataOutputStream dos, Response response) throws IOException {
	...
	private void setContentLength(DataOutputStream dos, Response response) throws IOException {
	...
	private void responseBody(DataOutputStream dos, Response response) {
		...
		try {
			dos.write(response.getContent(), 0, response.getContent().length);
			dos.flush();
		} catch (IOException e) {
			log.error(e.getMessage());
		}
